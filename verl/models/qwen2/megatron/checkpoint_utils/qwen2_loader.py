...
def _fetch_tensor(tensor, name, state_dict) -> torch.Tensor:
    """fetch tensor"""
    if tensor is not None:
        tensor = tensor.data.copy_(state_dict[name], non_blocking=True)


def _fetch_tp_shard_tensor_vocab(tensor, name, state_dict, mpu, chunk_dim=0, mutate_func=None) -> torch.Tensor:
...
def _fetch_tp_shard_tensor(tensor, name, state_dict, mpu, chunk_dim=0, mutate_func=None) -> torch.Tensor:
    """fetch tensor in tp shards"""
    tp_rank = mpu.get_tensor_model_parallel_rank()
    tp_size = mpu.get_tensor_model_parallel_world_size()
    if name in state_dict:
...
def _fetch_tp_shard_tensor_gate_up(tensor, gate_name, up_name, state_dict, mp_group, mpu, config, params_dtype) -> torch.Tensor:
...
def _fetch_tp_shard_tensor_qkv(tensor, q_name, k_name, v_name, state_dict, mp_group, mpu, config, params_dtype, bias=False) -> torch.Tensor:
...
def _load_embeddings(models, pp_rank, state_dict, mpu, print_rank_0):
    ...
    if pp_rank == 0:
        ...
        _fetch_tp_shard_tensor_vocab(embed_tokens_weight, "model.embed_tokens.weight", state_dict, mpu)


def _load_transformer_layer(layer, layer_map, models, pp_rank, offset, num_layer_this_model, state_dict, mp_group, mpu, config, params_dtype):
...
def _load_final_layernorm_and_lm_head(models, pp_rank, pp_size, state_dict, mpu, print_rank_0, is_value_model, tie_word_embeddings):
        ...
        if pp_rank + 1 == pp_size:
            ...
            if is_value_model:
                if "lm_head.weight" in state_dict and state_dict["lm_head.weight"].shape[0] == 1:
                    _fetch_tensor(lm_head_weight, "lm_head.weight", state_dict)
                    ...
                    _fetch_tensor(lm_head_weight, "reward_head.weight", state_dict)
                    print_rank_0("load lm_head from value_head weight")
                else:
                    _fetch_tensor(None, "lm_head.weight", state_dict)
                    print_rank_0("fail to match lm_head in value_model")

            else:
                _fetch_tp_shard_tensor(lm_head_weight, "lm_head.weight", state_dict, mpu)
...
def load_state_dict_to_megatron_qwen2(state_dict, wrapped_models, config, params_dtype, is_value_model=False, tie_word_embeddings=False):
    ...
    # Embeddings
    # -------------------
    _load_embeddings(models, pp_rank, state_dict, mpu, print_rank_0)
    ...
    for layer in layer_list:
        _load_transformer_layer(layer, layer_map, models, pp_rank, offset, num_layer_this_model, state_dict, mp_group, mpu, config, params_dtype)
    # Final Layernorm
    # -------------------
    _load_final_layernorm_and_lm_head(models, pp_rank, pp_size, state_dict, mpu, print_rank_0, is_value_model, tie_word_embeddings)
