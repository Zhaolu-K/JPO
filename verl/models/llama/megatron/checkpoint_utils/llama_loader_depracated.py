...
def _broadcast_tensor(tensor, name, state_dict, params_dtype, mp_group) -> torch.Tensor:
...
def _broadcast_tp_shard_tensor_vocab(tensor, name, state_dict, params_dtype, mp_group, chunk_dim=0, mutate_func=None) -> torch.Tensor:
...
def _broadcast_tp_shard_tensor(tensor, name, state_dict, params_dtype, mp_group, chunk_dim=0, mutate_func=None) -> torch.Tensor:
    """broadcast tensor in tp shards across mp_group"""
    from megatron.core import mpu
...
def _broadcast_tp_shard_tensor_gate_up(tensor, gate_name, up_name, state_dict, config, params_dtype, mp_group) -> torch.Tensor:
    """broadcast tensor in tp shards across mp_group"""
    from megatron.core import mpu
...
def _broadcast_tp_shard_tensor_qkv(tensor, q_name, k_name, v_name, state_dict, config, params_dtype, mp_group) -> torch.Tensor:
    """broadcast tensor in tp shards across mp_group"""
    from megatron.core import mpu
...
def _broadcast_embeddings(gpt_model_module, pp_rank, state_dict, params_dtype, mp_group):
    """broadcast the embedding weights"""
    embed_tokens_weight = None
    if pp_rank == 0:
        embed_tokens_weight = gpt_model_module.model.embed_tokens.weight
    _broadcast_tp_shard_tensor_vocab(embed_tokens_weight, "model.embed_tokens.weight", state_dict, params_dtype, mp_group)


def _broadcast_transformer_layer(sync_layer, layer_name, dst_pp_rank, pp_rank, state_dict, config, params_dtype, mp_group):
...
def _broadcast_final_layernorm_and_lm_head(gpt_model_module, pp_rank, pp_size, is_value_model, state_dict, params_dtype, mp_group):
    """broadcast the final layernorm and lm_head weights"""
    from verl.utils.megatron_utils import print_rank_0
    ...
    if is_value_model:
        if "lm_head.weight" in state_dict and state_dict["lm_head.weight"].shape[0] == 1:
            _broadcast_tensor(lm_head_weight, "lm_head.weight", state_dict, params_dtype, mp_group)
            print_rank_0("load lm_head weight")
        elif "reward_head.weight" in state_dict and state_dict["reward_head.weight"].shape[0] == 1:
            _broadcast_tensor(lm_head_weight, "reward_head.weight", state_dict, params_dtype, mp_group)
            print_rank_0("load lm_head from value_head weight")
        else:
            _broadcast_tensor(None, "lm_head.weight", state_dict, params_dtype, mp_group)
            print_rank_0("fail to match lm_head in value_model")
    else:
        _broadcast_tp_shard_tensor(lm_head_weight, "lm_head.weight", state_dict, params_dtype, mp_group)
...
def load_state_dict_to_megatron_llama(state_dict, wrapped_models, config, params_dtype, is_value_model=False, tie_word_embeddings=False):
    ...
    if dp_rank == 0:
        ...
        _broadcast_embeddings(gpt_model_module, pp_rank, state_dict, params_dtype, mp_group)
        ...
        for layer in range(config.num_hidden_layers):
            ...
            _broadcast_transformer_layer(sync_layer, layer_name, dst_pp_rank, pp_rank, state_dict, config, params_dtype, mp_group)
        # Final Layernorm
        # -------------------
        print_rank_0("loading final layernorm...")
        gpt_model_module = _get_gpt_model(models[-1])
        _broadcast_final_layernorm_and_lm_head(gpt_model_module, pp_rank, pp_size, is_value_model, state_dict, params_dtype, mp_group)
    dist.barrier()
