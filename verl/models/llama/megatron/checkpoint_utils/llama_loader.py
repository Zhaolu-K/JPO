...
def _megatron_calc_layer_list(config, pp_rank, pp_size):
    """Calculate the list of global layer indices held by the current pipeline rank."""
    from megatron.core import mpu
...
def _fetch_tp_shard_tensor(tensor, name, state_dict, chunk_dim=0, mutate_func=None) -> torch.Tensor:
...
def _fetch_tp_shard_tensor_gate_up(tensor, gate_name, up_name, state_dict, config, params_dtype, mp_group) -> torch.Tensor:
...
def _fetch_tp_shard_tensor_qkv(tensor, q_name, k_name, v_name, state_dict, config, params_dtype, mp_group) -> torch.Tensor:
...
def _fetch_embeddings(gpt_model_module, pp_rank, state_dict):
    """fetch the embedding weights"""
    embed_tokens_weight = None
    if pp_rank == 0:
        embed_tokens_weight = gpt_model_module.model.embed_tokens.weight
    _fetch_tp_shard_tensor(embed_tokens_weight, "model.embed_tokens.weight", state_dict)


def _fetch_transformer_layer(sync_layer, layer_name, dst_pp_rank, pp_rank, state_dict, config, params_dtype, mp_group):
    """fetch the weights of a single transformer layer"""
    _fetch_tensor(
...
def _fetch_final_layernorm_and_lm_head(gpt_model_module, pp_rank, pp_size, is_value_model, state_dict):
    """fetch the final layernorm and lm_head weights"""
    ...
    if pp_rank + 1 == pp_size:
        ...
        if is_value_model:
            ...
            _fetch_tp_shard_tensor(lm_head_weight, "lm_head.weight", state_dict)
...
def load_state_dict_to_megatron_llama(state_dict, wrapped_models, config, params_dtype, is_value_model=False, tie_word_embeddings=False):
    ...
    _fetch_embeddings(gpt_model_module, pp_rank, state_dict)
    ...
    pp_rank = mpu.get_pipeline_model_parallel_rank()
    pp_size = mpu.get_pipeline_model_parallel_world_size()
    layer_list = _megatron_calc_layer_list(config, pp_rank, pp_size)
    ...
    for layer in layer_list:
        ...
        _fetch_transformer_layer(sync_layer, layer_name, dst_pp_rank, pp_rank, state_dict, config, params_dtype, mp_group)
    # Final Layernorm
    # -------------------
    print_rank_0("loading final layernorm...")
    gpt_model_module = _get_gpt_model(models[-1])
    _fetch_final_layernorm_and_lm_head(gpt_model_module, pp_rank, pp_size, is_value_model, state_dict)
