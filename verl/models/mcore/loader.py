...
def _broadcast_params(module, mpu):
    for param in module.parameters():
        torch.distributed.broadcast(param.data, src=mpu.get_data_parallel_src_rank(), group=mpu.get_data_parallel_group())


def _broadcast_tensor(tensor, name, src_rank, mp_group, state_dict, params_dtype, print_rank_0) -> torch.Tensor:
...
def _broadcast_tp_shard_tensor_vocab(tensor, name, mpu, src_rank, mp_group, state_dict, params_dtype, print_rank_0, chunk_dim=0, mutate_func=None) -> torch.Tensor:
...
def _broadcast_tp_shard_tensor(tensor, name, mpu, src_rank, mp_group, state_dict, params_dtype, print_rank_0, chunk_dim=0, mutate_func=None) -> torch.Tensor:
    """broadcast tensor in tp shards across mp_group"""
    tp_rank = mpu.get_tensor_model_parallel_rank()
    tp_size = mpu.get_tensor_model_parallel_world_size()
...
def _broadcast_tp_shard_tensor_gate_up(tensor, gate_name, up_name, mpu, src_rank, mp_group, state_dict, config, params_dtype, print_rank_0) -> torch.Tensor:
    """broadcast tensor in tp shards across mp_group"""
    tp_rank = mpu.get_tensor_model_parallel_rank()
    tp_size = mpu.get_tensor_model_parallel_world_size()
...
def _broadcast_tp_shard_tensor_qkv(tensor, q_name, k_name, v_name, mpu, src_rank, mp_group, state_dict, config, models, params_dtype, print_rank_0, bias=False) -> torch.Tensor:
    """broadcast tensor in tp shards across mp_group"""
    tp_rank = mpu.get_tensor_model_parallel_rank()
    tp_size = mpu.get_tensor_model_parallel_world_size()
...
def _init_parallel_state(mpu):
    """Read the parallel ranks/sizes used for loading and check them on the source rank."""
    ...
    return dp_rank, pp_rank, src_rank, pp_size, virtual_pp_size, mp_group


def _unwrap_and_check_models(wrapped_models, config, pp_size, virtual_pp_size, unwrap_model, torchDDP, LocalDDP, Float16Module):
    """Unwrap the models of this rank and check they hold the expected number of layers.

    Returns the (possibly normalized) wrapped_models together with the unwrapped models.
    """
    ...
    return wrapped_models, models


def _load_embeddings(pp_rank, mpu, src_rank, mp_group, state_dict, models, params_dtype, print_rank_0):
    """broadcast the word embedding weights"""
    ...
    _broadcast_tp_shard_tensor_vocab(embed_tokens_weight, "model.embed_tokens.weight", mpu, src_rank, mp_group, state_dict, params_dtype, print_rank_0)


def _load_one_layer(layer, layer_map, pp_rank, mpu, src_rank, mp_group, state_dict, config, models, params_dtype, print_rank_0):
    """broadcast the weights of a single transformer layer"""
    ...
    _broadcast_tensor(
        ...
        src_rank, mp_group, state_dict, params_dtype, print_rank_0,
    )
    ...
    _broadcast_tp_shard_tensor_qkv(
        ...
        mpu, src_rank, mp_group, state_dict, config, models, params_dtype, print_rank_0,
    )
    ...
    _broadcast_tp_shard_tensor(
        ...
        mpu, src_rank, mp_group, state_dict, params_dtype, print_rank_0,
        chunk_dim=1,
    )
    _broadcast_tensor(
    ...
    _broadcast_tp_shard_tensor_gate_up(
        ...
        mpu, src_rank, mp_group, state_dict, config, params_dtype, print_rank_0,
    )
...
def _load_transformer_layers(pp_rank, mpu, src_rank, mp_group, state_dict, config, models, params_dtype, print_rank_0):
    """broadcast every transformer layer to the rank that owns it"""
    layer_map = _megatron_calc_layer_map(config)

    for layer in range(config.num_hidden_layers):
        _load_one_layer(layer, layer_map, pp_rank, mpu, src_rank, mp_group, state_dict, config, models, params_dtype, print_rank_0)


def _load_final_layernorm_and_lm_head(pp_rank, pp_size, is_value_model, mpu, src_rank, mp_group, state_dict, models, params_dtype, print_rank_0):
    """broadcast the final layernorm and the lm_head weights"""
    ...
    if is_value_model:
        ...
        if "lm_head.weight" in state_dict and state_dict["lm_head.weight"].shape[0] == 1:
            _broadcast_tensor(lm_head_weight, "lm_head.weight", src_rank, mp_group, state_dict, params_dtype, print_rank_0)
        elif "reward_head.weight" in state_dict and state_dict["reward_head.weight"].shape[0] == 1:
            _broadcast_tensor(lm_head_weight, "reward_head.weight", src_rank, mp_group, state_dict, params_dtype, print_rank_0)
            print_rank_0("load lm_head from value_head weight")
        else:
            _broadcast_tensor(None, "lm_head.weight", src_rank, mp_group, state_dict, params_dtype, print_rank_0)
        ...
        _broadcast_tp_shard_tensor(lm_head_weight, "lm_head.weight", mpu, src_rank, mp_group, state_dict, params_dtype, print_rank_0)


def _broadcast_params_in_dp_groups(wrapped_models, mpu):
    """broadcast the loaded weights from the dp source rank to the whole dp group"""
    for wrapped_model in wrapped_models:
        _broadcast_params(wrapped_model, mpu)
...
def load_state_dict_to_megatron_gptmodel(state_dict, wrapped_models, config, params_dtype, is_value_model=False):
    ...
    dp_rank, pp_rank, src_rank, pp_size, virtual_pp_size, mp_group = _init_parallel_state(mpu)

    wrapped_models, models = _unwrap_and_check_models(wrapped_models, config, pp_size, virtual_pp_size, unwrap_model, torchDDP, LocalDDP, Float16Module)

    if dp_rank == 0:
        # Embeddings
        # -------------------
        _load_embeddings(pp_rank, mpu, src_rank, mp_group, state_dict, models, params_dtype, print_rank_0)

        # Transformer layers
        # -------------------
        _load_transformer_layers(pp_rank, mpu, src_rank, mp_group, state_dict, config, models, params_dtype, print_rank_0)

        # Final Layernorm
        # -------------------
        _load_final_layernorm_and_lm_head(pp_rank, pp_size, is_value_model, mpu, src_rank, mp_group, state_dict, models, params_dtype, print_rank_0)
    dist.barrier()
    # Broadcast weights inside data parallel groups
    _broadcast_params_in_dp_groups(wrapped_models, mpu)
    pass
