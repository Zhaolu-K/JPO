...
def compute_policy_loss(
    ...
    cliprange_low=None,
    cliprange_high=None,
    weight_per_token=None,
    advantage_per_seq=None,
    clip_ratio_c=3.0,
    ...
    cliprange_tensor = torch.full_like(ratio, cliprange)
    for i in range(len(weight_per_token)):
        for j in range(len(weight_per_token[i])):
            cliprange_tensor[i, j] = cliprange * compute_token_norm_weight(
                weight_per_token[i][j], advantage_per_seq[i])

    pg_losses2 = -advantages * torch.clamp(ratio, 1 - cliprange_tensor, 1 + cliprange_tensor)
